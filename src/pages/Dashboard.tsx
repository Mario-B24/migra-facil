...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    await Promise.all([
      ...
      fetchExpedientesPorMes(),
      fetchIngresosPorMes(),
      fetchTopTiposTramite(),
      fetchExpedientesRecientes(),
      fetchClientesConNieVenciendo(),
      fetchExpedientesSinMovimiento(),
    ]);
  };
  ...
  const fetchStats = async () => {
    ...
    const [clientes, expedientesActivos, expedientesPendientes, pagos] = await Promise.all([
      ...
      supabase
        .from("payments")
        .select("importe")
        .gte("fecha_pago", format(firstDayOfMonth, "yyyy-MM-dd"))
        .lte("fecha_pago", format(lastDayOfMonth, "yyyy-MM-dd")),
    ]);
  ...
  const fetchExpedientesPorEstado = async () => {
    const { data } = await supabase.from("expedientes").select("estado");
  ...
  const fetchExpedientesPorMes = async () => {
    const { data } = await supabase
      .from("expedientes")
      .select("fecha_inicio")
      .gte("fecha_inicio", format(subMonths(new Date(), 5), "yyyy-MM-dd"))
    ...
    data.forEach((exp) => {
      const monthKey = format(new Date(exp.fecha_inicio), "MMM yy", { locale: es });
  ...
  const fetchIngresosPorMes = async () => {
    const { data } = await supabase
      ...
      .gte("fecha_pago", format(subMonths(new Date(), 5), "yyyy-MM-dd"))
    ...
    data.forEach((pago) => {
      const monthKey = format(new Date(pago.fecha_pago), "MMM yy", { locale: es });
  ...
  const fetchTopTiposTramite = async () => {
    const { data } = await supabase
      .from("expedientes")
      .select("tipo_tramite_id, tipos_tramite!inner(nombre)");
    ...
    const chartData = Object.entries(tramiteCounts)
  ...
  const fetchClientesConNieVenciendo = async () => {
    const { data } = await supabase
      .from("clients")
      .select("id, nombre, apellidos, nie_pasaporte, fecha_vencimiento_nie")
      .not("fecha_vencimiento_nie", "is", null);
    ...
    const alertas = data
      ...
      .filter((c) => c.dias_restantes <= 30 && c.dias_restantes >= 0)
  ...
  const fetchExpedientesSinMovimiento = async () => {
    ...
    const sinMovimiento = data
      ...
      .filter((e) => e.dias_sin_movimiento > 30)
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.ingresosMesActual.toFixed(2)} €</div>
            </CardContent>
          </Card>
        </div>

        {/* Alertas */}
        {(clientesConNieVenciendo.length > 0 || expedientesSinMovimiento.length > 0) && (
          <div className="grid gap-4 md:grid-cols-2">
            {clientesConNieVenciendo.length > 0 && (
              <Card>
                <CardHeader>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {clientesConNieVenciendo.map((cliente) => (
                      <div
                        key={cliente.id}
            ...
            {expedientesSinMovimiento.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {expedientesSinMovimiento.map((exp) => (
                      <div
                        key={exp.id}
                        className="flex justify-between items-center p-2 bg-red-50 rounded border border-red-200 cursor-pointer hover:bg-red-100"
                ...
                <PieChart>
                  ...
                  >
                    {expedientesPorEstado.map((entry, index) => {
                      const estado = Object.keys(ESTADO_LABELS).find(
                        (k) => ESTADO_LABELS[k] === entry.estado
                      );
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {topTiposTramite.map((tramite, index) => (
                      ...
                      <div className="w-full bg-secondary h-2 rounded-full">
                        <div
                          ...
                          style={{
                            width: `${
                              (tramite.cantidad /
                                Math.max(...topTiposTramite.map((t) => t.cantidad))) *
                              100
                            }%`,
                          }}
                        />
